...
class APIManager: SessionManager {
    ...
    static let consumerKey = "lzIfri5ACGz8wpBXFZMDdNfn5"
    static let consumerSecret = "LnyrsmWe1YGIdUIonHLHx5A93YUd8dKtnajWCUL1lX5OvU93BK"
    ...
    func retweetTweet(with tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        ...
        let baseURL = "https://api.twitter.com/1.1/statuses/retweets/"
        let id: Int64 = tweet.id
        let idString = String(id)
        let url = baseURL + idString + ".json"
        print(url)
        
        request(url, method: .post, parameters: paramters, encoding: URLEncoding.queryString).validate().responseJSON { (response) in
    ...
    // MARK: Likes
//    func likeTweet(with tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
//        let paramters = ["id": tweet.id]
//        
//        let baseURL = "https://api.twitter.com/1.1/statuses/favorite/"
//        let id: Int64 = tweet.id
//        let idString = String(id)
//        let url = baseURL + idString + ".json"
//        print(url)
